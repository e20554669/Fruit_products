...
class WeatherDataProcessor:
        ...
        try:
            ...
            if "Data" in data:
                ...
                if len(weather_data) > 0:
                    print(f"[範例] 第一筆資料鍵值: {list(weather_data[0].keys())}")
                    print(f"[範例] 第一筆城市: {weather_data[0].get('CITY', 'N/A')}")
                    print(f"[範例] 第一筆溫度: {weather_data[0].get('TEMP', 'N/A')}")

                return weather_data
            else:
                print(f"[錯誤] API回應中沒有Data欄位")
                print(
                    f"[除錯] 完整回應內容: {json.dumps(data, ensure_ascii=False, indent=2)}"
                )
                return []
            # ==============================
    ...
    def calculate_city_averages(
        ...
        # 檢查颱風狀況
        if target_date:
            check_date = target_date.replace("/", "-")
    ...
    def process_single_date(self, date_str: str) -> str:
        ...
        try:
            ...
            print(f"\n[摘要] {date_str} 處理結果:")
            print(
                f"   處理縣市數: {len(city_averages)}"
            )
            print(
                f"   平均溫度範圍: {city_averages['Temperature'].min():.1f}°C - {city_averages['Temperature'].max():.1f}°C"
            )
            print(
                f"   平均濕度範圍: {city_averages['RH'].min():.1f}% - {city_averages['RH'].max():.1f}%"
            )
...
def main():
    ...
    # ============== 日期設定區 ==============
    # 直接使用「字串」日期；若改成 datetime 也 OK（函式已兼容）
